...
搜索一个服务号并关注发私信：
  wechat_search("火眼审阅")
  → 看返回：如果是"搜索结果"页面 → wechat_click_first_result()
  → 看返回：如果是"详情页"且有"关注"按钮 → wechat_click_button("关注")
  → 看返回：已关注成功有"私信"按钮 → wechat_click_button("私信")
  → 看返回：进入聊天窗口 → wechat_type_and_send("测试消息")
  → 看返回：消息发送成功 → 汇报结果

如果某一步返回的不是预期状态：
  - 如果是登录页面 → 告诉用户需要先登录
  - 如果已关注 → 跳过关注直接点私信
  - 如果按钮定位失败 → 读 OCR 文字判断是否页面变化
