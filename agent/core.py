...
5. 返回文件路径后 done(success=true)

⚠️ 不要操作浏览器（不要 navigate/click/input）。
"""
...
## 可用工具（直接作为 action key 调用，不是 JavaScript 函数！）
...
## 调用格式（重要！）
这些是 Python 工具函数，必须直接作为 action key 放在 JSON 中，不要包在 evaluate/navigate 里！！！

正确示例：
{"action":[{"wechat_search_and_follow":{"keyword":"火眼审阅","message":"你好","account_type":"服务号"}}]}

错误示例（严禁这样做）：
❌ {"action":[{"evaluate":{"code":"wechat_search_and_follow(...)"}}]}
❌ {"action":[{"navigate":{"url":"javascript:wechat_search_and_follow(...)"}}]}
...
⚠️ 不要操作浏览器，唯一能用的就是这两个微信工具。禁止使用 evaluate/navigate/go_back/switch。
...
async def create_wps_agent(task: str) -> Agent:
    ...
    wps_tools.registry.action(
        description="查询指定类型（周报/会议纪要/报告/通知/计划/总结/简历/文章）的WPS文档模板缓存，返回上次使用的排版参数和章节结构骨架。命中后直接复用，跳过排版推理和内容结构设计。"
    )(get_wps_template)
    wps_tools.registry.action(
        description="启动WPS新建文字文档，写入标题和正文(Markdown格式)，设置字体/段落/编号，保存.docx并导出PDF。完成后返回文件路径。"
