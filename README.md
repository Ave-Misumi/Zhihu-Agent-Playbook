...
| **微信** | 搜索公众号/服务号 → 关注 → 发送私信 | pyautogui + OpenCV 视觉定位 + LLM Agent |
...
```
Zhihu-Agent-Playbook/
├── main.py                     # 入口：意图路由 + 三链路分发（微信流式输出）
├── config.py                   # LLM API / 输出解析容错（BridgeLLM + streaming）
...
│   ├── wechat.py               # 微信自动化：搜索/关注/发私信（旧版 RPA，保留）
│   ├── wechat_agent.py         # 微信 Agent 层：5 个原子工具 + 重试/人工辅助/Playbook
...
- **LangChain 1.3.13+** — LLM 调用与 ReAct Agent（`create_agent` / LangGraph StateGraph）
