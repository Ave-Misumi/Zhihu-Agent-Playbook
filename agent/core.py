...
**朋友圈点赞（LLM 自主决策，不是 RPA）：**
  **关键：必须先回到微信主窗口！**
  ...
  5. 看返回结果：
     - 「已给第1条朋友圈点赞」→ 任务完成
     - 「未找到「两个点」按钮」→ 汇报给用户，不要尝试用 wechat_click_text 找「赞」或「...」
     - 失败 → 汇报给用户
  ...
  **重要提醒：**
  ...
  - **禁止用 wechat_click_text("赞") 手动点「赞」**——电脑版微信朋友圈的「赞」只在点击「两个点」按钮后弹出的菜单中出现，不会直接显示在页面上
  - 只有「朋友圈」等导航文字可以用 wechat_click_text
