...
**朋友圈点赞（LLM 自主决策，不是 RPA）：**
  1. 打开微信恢复主窗口 → **左侧侧边栏直接找「朋友圈」图标**
     - 使用模板匹配**：wechat_click_text("朋友圈图标")（内部会自动用模板匹配）
  2. 在朋友圈 → **不要滚动！第一条动态就在顶部**
  3. wechat_like_moments_post(n=1) → 自动完成：找「两个点」按钮 → 点击 → 点「赞」
  4. 看返回结果：
     - 「已给第1条朋友圈点赞」→ 任务完成
  ...
  **任务完成判断（重要！）：**
  - wechat_like_moments_post 会自动验证：点赞后会再次打开菜单，检查是否出现「取消」字样
  - 如果返回「✅ 已给第1条朋友圈点赞（验证：菜单显示「取消」）」→ **任务完成！**
  - 如果返回「⚠️ 验证未通过」→ 可能点赞失败，需要重试或手动处理
  - 不要继续尝试点赞，直接报告任务完成
  
  **重要提醒：**
  - 「两个点」按钮（⋯）是**图标**，不是文字！不要用 wechat_click_text("...") 去点它，会失败
  - 正确做法：始终用 wechat_like_moments_post(n=1) 自动处理，它会用模板匹配找到图标
  - 只有「赞」「评论」「朋友圈」等才是文字，可以用 wechat_click_text
...
## 信息提取技巧（关键！）
- **优先用 innerText 获取全部文字再分析**：`document.body.innerText` 或 `document.querySelector('main')?.innerText` —— 这是最简单可靠的方式
- **如果 DOM 选择器不确定，先用 innerText 获取可见文字**，从中分析出需要的信息，而不是反复尝试可能错误的选择器
- **对于列表类内容（如职位列表、商品列表）**：
  - 先用 `document.body.innerText` 获取页面全部文字，从中找到列表项的文本
  - 或者尝试用通用选择器如 `document.querySelectorAll('li')`、`document.querySelectorAll('[class*="job"]')`、`document.querySelectorAll('[class*="item"]')` 等
  - 如果通用选择器也失败，**直接点击列表项进入详情页**，用 innerText 提取详情
- **提取结构化数据**：`Array.from(document.querySelectorAll('选择器')).map(el => el.innerText.trim()).filter(t => t)`
- **翻页后 wait 2秒等页面加载,再提取新数据**

## BOSS直聘/招聘网站专用策略
- **列表页信息有限**：搜索结果列表通常只显示职位名称、公司、薪资、地点等基本信息
- **获取详细信息的方法**：
  1. 先记录列表页看到的基本信息（职位名、公司、薪资、地点）
  2. **逐个点击职位进入详情页**，在详情页用 `document.body.innerText` 提取完整职位描述、要求、福利等
  3. 点击浏览器的"返回"按钮回到列表页，继续下一个
  4. 收集完所需数量的职位详情后，进行总结分析
- **BOSS直聘页面特点**：
  - 点击职位后可能是右侧弹出详情抽屉（不跳转页面），此时直接在当前页用 `document.body.innerText` 提取
  - 如果点击后没有明显变化，wait 2秒后直接用 `document.body.innerText` 提取当前页所有文字
  - **不要尝试关闭弹窗或找返回按钮**——直接提取当前可见内容即可
- **不要反复用 querySelectorAll 尝试提取列表项**——如果2次尝试都失败，直接改用 innerText 或点击详情页
- **只使用可用的工具**：click, input, navigate, scroll, wait, evaluate, go_back, done, ask_human_for_intervention, wps_create_document_and_export_pdf。**不要使用 extract 工具，它不存在！**
...
## 行为准则
- 读懂用户的自然语言,拆解为先后步骤,逐一执行
- **信息提取失败时换策略**：如果 querySelectorAll 2次都失败，立刻改用 innerText 获取全部文字再分析
- 每进入新页面后 wait 2秒,观察是否有弹窗,有关闭掉再继续
- 同一个 wait 操作不要连续超过 2 次
- 所有任务完成后调用 done(success=true),不要继续浏览
...
def _kill_stale_edge_for_profile(user_data_dir: str):
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        ...
        except (psutil.NoSuchProcess, psutil.AccessDenied, ProcessLookupError, PermissionError, OSError):
            pass
    if killed > 0:
